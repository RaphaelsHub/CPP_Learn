...
public:
    ...
    Beer(string typeBeer, string color, double weight)
    {
        nameBeer = typeBeer;
        colorBeer = color;
        weightBeer = weight;
...
void Buyer::BuyBeer(Beer &typeOfBeer)
{
...
public:
    ...
    void BuyFruit(const Ananas &typeOfFruit);
};
...
private:
    ...
    friend void Customer::BuyFruit(const Ananas &typeOfFruit);
public:
...
inline void Customer::BuyFruit(const Ananas &typeOfFruit)
{
...
inline void CheckFriends()
{
    ...
    Customer PekelAlexandr;

    PekelAlexandr.BuyFruit(fruit);

    Beer Beer{"apple", "green", 234};
    Buyer PekelAlexandrr;

    PekelAlexandrr.BuyBeer(Beer);
